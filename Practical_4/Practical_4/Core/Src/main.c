...
int main(void)
{
  ...
  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Sin_LUT, NS);
...
static void MX_TIM2_Init(void)
{
  ...
  htim2.Init.Period = TIM2_Ticks - 1;//4294967295
...
void EXTI0_IRQHandler(void){
	...
	// HINT: Consider using C's "switch" function to handle LUT changes
	  __HAL_TIM_DISABLE_DMA(&htim2, TIM_DMA_CC1);      // disable DMA request from TIM2 CC1
	  HAL_DMA_Abort_IT(&hdma_tim2_ch1);                // abort ongoing DMA transfer (non-blocking, sends abort)

	  // Changing waves on button press
	  wave_index++;
	  if (wave_index >= 7){
		  wave_index = 1; // Loop back to 1 after reaching final index
	  }

	  switch (wave_index){
	  case 1:
		  // Corresponds to sine wave
		  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Sin_LUT, NS); // Start DMA
		  lcd_update_line(LCD_CHARS[0]);
		  break;

	  case 2:
		  // Corresponds to Sawtooth wave
		  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Saw_LUT, NS);
		  lcd_update_line(LCD_CHARS[1]);
		  break;

	  case 3:
		  // Corresponds to Triangular wave
		  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Triangle_LUT, NS);
		  lcd_update_line(LCD_CHARS[2]);
		  break;

	  case 4:
		  // Corresponds to Guitar wave
		  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Guitar_LUT, NS);
		  lcd_update_line(LCD_CHARS[3]);
		  break;

	  case 5:
		  // Corresponds to piano wave
		  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Piano_LUT, NS);
          lcd_update_line(LCD_CHARS[4]);
          break;

	  case 6:
		  // Corresponds to drum wave
		  HAL_TIM_OC_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)Drum_LUT, NS);
		  lcd_update_line(LCD_CHARS[5]);
		  break;
