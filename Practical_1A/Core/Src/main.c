...
uint8_t ledPattern_Mode3;// Initialized in main while loop.
int Mode3_displaydelay;
int Mode3_turnoffdelay;
int Mode3_displaying = 0;

// direction and index used to cycle through the led pattern
int direction = 1;
int index = 0;
int displayMode = 0;
int currentDelay = 0;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // TODO: Check pushbuttons to change timer delay
	  if (HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin) == 0)// i used original variable name
	  {
		  if (currentDelay == 0)
		  {
...
void TIM16_IRQHandler(void)
{
	...
	if (HAL_GPIO_ReadPin(Button1_GPIO_Port, Button1_Pin) == 0 || HAL_GPIO_ReadPin(Button2_GPIO_Port, Button2_Pin) == 0 || HAL_GPIO_ReadPin(Button3_GPIO_Port, Button3_Pin) == 0)
	{
		if (!currentDelay)
		{
			__HAL_TIM_SET_AUTORELOAD(&htim16, 999);
...
// switch case for the three modes
	switch (displayMode){
	case 1:
		// Turn off all leds

		for (int i = 0; i < 8; i++) {
		        HAL_GPIO_WritePin(LED0_GPIO_Port, ledPins[i], GPIO_PIN_RESET); // No reason to change LED0_GPIO_Port since they map to the same thing
		    }

		HAL_GPIO_WritePin(LED0_GPIO_Port, ledPins[index], GPIO_PIN_SET);

		// Update index with back/forth logic
		index += direction;
		if (index > 7 || index < 0) {
			direction *= -1;
			index += 2*direction; // Skip re-showing the same LED
		}
		break;
	...
	case 2:
		...
		if (index > 7 || index < 0) {
			direction *= -1;
			index += 2*direction; // Skip re-hiding the same LED
		}
		break;
	...
	case 3:
		...
		if (!Mode3_displaying)
		{
			...
			for (int i = 0; i < 8; i++) {
				...
				if (state) {
					...
					HAL_GPIO_WritePin(LED0_GPIO_Port, ledPins[i],
							GPIO_PIN_RESET);
				}
			}
			// put display delay
		...
		else
		{
			...
			//put turn off delay between cycles
